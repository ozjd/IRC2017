; MSN Auth (UTF-8 compatible) by JD. Requires mIRC v7.44
...
on *:PARSELINE:*:*:{
  if ($parsetype === in) {
    ...
    if (($1- === PASS) && ($chr(37) $+ #* iswm $active)) { .parseline -ot | raw -q MODE $me +h $$?="Please enter the host keyword" }
    elseif ($1 === JOIN) { var %i = 1, %r, %s, %c | while (%i <= $numtok($2,44)) { %c = $iif(%i !== 1,$chr(44)) | %d = $gettok($2,%i,44) | %r = $+(%r,%c,%d) | %s = $+(%s,%c,$iif($readini(irc7.ini,keys,$+($server,$chr(44),%d,$chr(44),OWNERKEY)), $v1,$iif($readini(irc7.ini,keys,$+($server,$chr(44),%d,$chr(44),HOSTKEY)), $v1, $gettok($3,%i,44)))) | inc %i } | .parseline -otn $1 %r %s }
  }
}
...
on 1:sockread:jd.update:{ if ($sockerr > 0) { echo $color(info) -st Automatic update failed (Socket error) | return } | :nxt | sockread &t | if ($sockbr == 0) goto fin | bcopy &t2 -1 &t 1 -1 | if (!$sock($sockname).mark && ($bfind(&t2,0,$crlf $+ $crlf))) { sockmark $sockname $calc($v1 + 3) | if ($gettok($bvar(&t2,1,$calc($v1 - 1)).text,2,32) !== 200) { echo $color(info) -st * Automatic update failed (HTTP Status != 200) | sockclose $sockname | return } } | goto nxt | :fin | if ($sock($sockname).mark > -1) {
    ...
    sockmark $sockname 0
} | else { echo $color(info) -st * Automatic update failed (Parsing error) | sockclose $sockname } }

on 1:sockclose:jd.update:{
  if ($sockerr > 0) {
    echo $color(info) -st * Automatic update failed (Premature disconnect)
  }
  else {
